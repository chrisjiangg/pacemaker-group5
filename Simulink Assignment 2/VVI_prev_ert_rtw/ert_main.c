...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(VVI_prev_M) == (NULL));
  while (runModel) {
    mw_osSignalEventWaitEver(&baserateTaskSem);
    VVI_prev_step();

    /* Get model outputs here */
    stopRequested = !((rtmGetErrorStatus(VVI_prev_M) == (NULL)));
  }
...
{
  UNUSED(sig);
  rtmSetErrorStatus(VVI_prev_M, "stopping the model");
  runModel = 0;
}
...
int main(int argc, char **argv)
{
  ...
  hardware_init();
  rtmSetErrorStatus(VVI_prev_M, 0);
