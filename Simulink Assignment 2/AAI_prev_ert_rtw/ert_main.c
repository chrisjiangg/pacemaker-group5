...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(AAI_prev_M) == (NULL));
  while (runModel) {
    mw_osSignalEventWaitEver(&baserateTaskSem);
    AAI_prev_step();

    /* Get model outputs here */
    stopRequested = !((rtmGetErrorStatus(AAI_prev_M) == (NULL)));
  }
...
{
  UNUSED(sig);
  rtmSetErrorStatus(AAI_prev_M, "stopping the model");
  runModel = 0;
}
...
int main(int argc, char **argv)
{
  ...
  hardware_init();
  rtmSetErrorStatus(AAI_prev_M, 0);
